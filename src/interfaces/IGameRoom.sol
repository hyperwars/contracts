...
interface IGameRoom {
    ...
    struct GameConfig {
        ...
        uint256 slicerIncentive; // bps (500 = 5%); reserved for the future bounty layer
        uint256 maxRoundDuration;
        uint256 joinWindow; // blocks
        uint8 minPlayers;
        uint8 maxPlayers;
        uint64 builderFeeRate;
        uint256 prizePoolShare; // bps (2000 = 20%); reserved for the future treasury layer
    ...
    event AgentAuthorized(address indexed player, address indexed proxy);
    ...
    function authorizeAgent(address player) external;
