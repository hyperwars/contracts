...
contract GameRoomActivateTest is GameRoomBaseTestBase {
    ...
    function test_authorizeAgent_allowedWhenFinished() public {
        _fillAndStart();
        _authorize(alice);
        _finishRound();
        address proxy = room.playerProxy(alice);
        address agent = _agentOf(alice);
        uint256 deadline = block.timestamp + 1 days;
        bytes memory sig = _signAuthorizeAgent(alice, agent, room.agentNonce(alice), deadline);
        vm.expectEmit(true, true, false, true);
        emit IGameRoom.AgentAuthorized(alice, proxy, agent);
        room.authorizeAgent(alice, agent, deadline, sig);
        assertEq(room.agentNonce(alice), 2);
    ...
    function test_authorizeAgent_revertsInLobby() public {
        _register(alice);
        address agent = _agentOf(alice);
        uint256 deadline = block.timestamp + 1 days;
        bytes memory sig = _signAuthorizeAgent(alice, agent, 0, deadline);
    ...
    function test_authorizeAgent_revertsWhenCancelled() public {
        _register(alice);
