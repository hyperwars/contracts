...
contract IntegrationTest is MockHyperCore {
    ...
    function _signAuthorizeAgent(address roomAddr, address player, address agent, uint256 nonce, uint256 deadline)
        internal
        view
        returns (bytes memory)
    {
        ...
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(playerPk[player], digest);
    ...
    function test_happyPath_freePlay_everyoneKeepsCapital() public {
        ...
        for (uint256 i; i < players.length; ++i) {
            address agent = makeAddr(string.concat("agent:", vm.toString(players[i])));
            uint256 deadline = block.timestamp + 1 days;
            bytes memory sig = _signAuthorizeAgent(address(room), players[i], agent, 0, deadline);
            vm.prank(keeper2);
            room.authorizeAgent(players[i], agent, deadline, sig);
        }
        CoreSimulatorLib.nextBlock();
